...
func (tx *Tx) init() {
	...
	tx.Entrenador = NewEntrenadorClient(tx.config)
	tx.Membresia = NewMembresiaClient(tx.config)
	tx.Rutina = NewRutinaClient(tx.config)
...
// applies a query, for example: Asistencia.QueryXXX(), the query will be executed
